...
export function renderNotifications(input, state = initializeNotificationState(input.items ?? notifications)) {
  const items = deriveNotifications(input, state).map((item) => {
    if (item.restricted) return `<article class="notification-row restricted-notification"><span class="restricted-symbol" aria-hidden="true">•</span><div><strong>Restricted network activity</strong><p>Identity and target are unavailable under the social visibility policy.</p><time>Local synthetic activity</time></div></article>`;
