...
const normalizeResolvedCapability = (
  value,
  participants
) => {
  const resolved =
...
export function createOpaqueRecipientCapabilityResolver({
  ...
  if (
    ...
    throw new TypeError(
      "invalid opaque recipient resolver dependencies"
    );
  }
  ...
  const resolve = async (input) => {
    ...
    try {
      ...
      const resolved =
        normalizeResolvedCapability(
          capabilityResolve.call(
            capabilityStore,
            {
              subject:
                session.subject,
