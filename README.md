...
Both read adapters are protocol seams, not production connectivity or authentication. Live production relay/runtime selection, signing, publishing, DMs, encryption, NIP-07, private-key access, HODLXXI authentication, CRT issuance, covenant or chain validation, sponsor/status mutation, writes to HODLXXI, and deployment are not implemented. The current viewer remains explicitly injected local state and does not establish key ownership.
