...
An independent, Nostr-first social layer for the HODLXXI covenant trust network. Identity is a public key—not a username, password, KYC record, or application-issued label. V0.1 is a dependency-free interactive product shell that works offline with synthetic fixtures.
...
Open `web/index.html` through any static file server, then switch among synthetic participants to explore Limited, Full, and Operator views. Viewer selection is in-memory only and cannot edit externally derived access status. Run `npm test` for deterministic Node built-in tests.
