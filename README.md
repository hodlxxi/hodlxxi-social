...
This foundation is not connected to the ordinary Social UI. No Social deployment, OAuth client registration, DNS, TLS, proxy, systemd service, or live login exists yet. Pending transactions and sessions are bounded, process-local, and non-durable; restart logs users out, and multiple processes require a separately reviewed shared-store design. OAuth authentication proves only the canonical subject. It grants no Full or Operator status, friendship, covenant or sponsor authority, or relay authority. NIP-07 selection remains separate routing context and is not an authenticated session. See [the V1.13 BFF contract](docs/SOCIAL_OAUTH_BFF_V1.md).
...
Not implemented: production live mode, automatic source or relay selection, relay discovery, relay pools, reconnect, persistent subscriptions, polling, personalized feeds, signing, publishing, DMs, encryption, NIP-07 signing or account management, application-integrated live HODLXXI authority, durable persistence, or deployment. The opt-in V1.13 BFF foundation is not connected to the ordinary UI and is not a registered live or production authentication service.
...
Not implemented: default live mode, relay discovery, relay pools, reconnect, persistent subscriptions, publishing, signing, DMs, encryption, NIP-07 signing or account management, NIP-44, NIP-59, NIP-65, production relay selection, application-integrated HODLXXI runtime transport, private-key access, live OAuth registration or login, CRT issuance, covenant or chain validation, sponsor/status mutation, writes to HODLXXI, or production deployment. Environment-based configuration and credential access are confined to the explicitly invoked server BFF; the ordinary browser UI has neither. The current viewer remains explicitly injected local state and does not establish key ownership or an authenticated session.
