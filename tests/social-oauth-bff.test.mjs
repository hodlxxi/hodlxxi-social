...
const setup=(oauthConfig=config)=>{let byte=0; const pendingTransactions=createBoundedStore({ttlSeconds:300,capacity:10,now:()=>0}); const sessions=createBoundedStore({ttlSeconds:3600,capacity:10,now:()=>0}); let tokenCalls=0,introspectionCalls=0; const codes=[]; const bff=createSocialOAuthBff({config:oauthConfig,pendingTransactions,sessions,oauthClient:{async authenticate({code}){tokenCalls++; codes.push(code); introspectionCalls++; return {subject:"a".repeat(64),accessToken:"human-oauth-bearer-private"};}},random:(size)=>Buffer.alloc(size,++byte)}); return {bff,pendingTransactions,sessions,codes,calls:()=>tokenCalls,tokenCalls:()=>tokenCalls,introspectionCalls:()=>introspectionCalls};};
...
test("login requests openid profile scopes from canonical configuration",async()=>{
  const canonicalConfig=parseSocialOAuthConfig({
    ...config,bindHost:"127.0.0.1",port:8080,
    maxPendingTransactions:10,maxSessions:10,outboundTimeoutMs:1000
  });
  const {bff}=setup(canonicalConfig);
  const result=await bff({method:"GET",url:"/auth/login",headers:{}});
  assert.equal(result.status,302);
  const redirect=new URL(result.headers.Location);
  assert.equal(redirect.origin,"https://authority.example");
