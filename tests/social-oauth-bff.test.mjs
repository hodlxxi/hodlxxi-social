...
import { createSocialOAuthBff } from "../src/server/social-oauth-bff.mjs"; import { createBoundedStore } from "../src/server/social-oauth-memory.mjs"; import { TRANSACTION_COOKIE_NAME, expireTransactionCookie } from "../src/server/social-oauth-cookie.mjs";
...
test("malformed raw callback query syntax cannot consume state or reach OAuth",async()=>{
  ...
  for(let index=0;index<hostileTargets("placeholder").length;index++){
    ...
    assert.equal(valid.status,200,url);
